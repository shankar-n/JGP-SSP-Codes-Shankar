...
% AUDIT(Claude-Fable 2026-06-10): warnbox env added — used by the repair notes
% inserted during verification; was missing from this preamble (compile error).
...
\begin{proofbox}
\begin{proof}
%% TODO-VERIFY(Claude-Fable 2026-06-10): proof tightened (blocks vs distinct configs).
A solution with $c$ configuration changes has $c+1$ maximal blocks of constant
configuration; the distinct configurations among the blocks cover all jobs by feasible
groups, so their number is $\ge\ZJGP$, giving $c+1\ge\ZJGP$, i.e.\ $c\ge\ZJGP-1$.
Conversely, processing a JGP-optimal grouping contiguously uses $\ZJGP-1$ changes.
So $\OPT_{\mathrm{TSIP}} = \ZJGP - 1$, achieved with exactly $\ZJGP$ groups. \qed
\end{proof}
\end{proofbox}
...
\begin{warnbox}
%% TODO-VERIFY(Claude-Fable 2026-06-10): logical repair. As stated, thm:sufficient is
%% VACUOUS: (C3) literally restates the definition of collapse, so "C1+C2+C3 implies
%% collapse" is trivial; and the old proof was additionally invalid (it asserted a
%% K*-group solution of cost exactly (K*-1)delta exists --- (C2) is only a LOWER
%% bound; the best K*-solution can be arbitrarily expensive). Replaced by the
%% non-trivial checkable condition below; the original statement is kept above for
%% reference but should be regarded as a definition-splitting device only.
\textbf{Repair: a non-vacuous sufficient condition.} (C3) restates the definition of
collapse, so \cref{thm:sufficient} as stated is empty. A \emph{checkable} sufficient
condition replaces (C3) by an upper bound matching (C2)'s lower bound:
\end{warnbox}

\begin{thmbox}
\begin{theorem}[Checkable sufficient condition for collapse]
\label{thm:sufficient_real}
Fix the convention that a solution's cost is summed over its block boundaries
(consecutive distinct configurations; intra-block transitions cost $0$ by (C1)).
Suppose $f$ satisfies (C1), the lower bound (C2) with constant $\delta>0$ for
\emph{all} pairs of distinct configurations, and the upper bound
\begin{description}
\item[(C2$'$)] some JGP-optimal grouping admits an order and configurations whose
  consecutive transition costs are all $\le\bar\Delta$.
\end{description}
If $\Kstar\,\delta\ \ge\ (\Kstar-1)\,\bar\Delta$, then SSP-$f$ collapses to JGP on the
instance.
\end{theorem}
\end{thmbox}
\begin{proofbox}
\begin{proof}
The (C2$'$) solution costs $\le(\Kstar-1)\bar\Delta$. Any solution with $K>\Kstar$
blocks has $K-1\ge\Kstar$ boundaries, each costing $\ge\delta$, hence costs
$\ge\Kstar\delta\ge(\Kstar-1)\bar\Delta$. So some $\Kstar$-group solution is at least
tied with every finer solution, and the optimum is achieved with $\ZJGP$ groups.
\end{proof}
\end{proofbox}

\begin{remark}[Application to $f_S$]
For the setup-time cost $f_S$: every change costs $\ge S_\mathrm{stop}+c_\mathrm{switch}$
($\ge1$ tool inserted) and the (C2$'$) bound is
$\bar\Delta\le S_\mathrm{stop}+b\,c_\mathrm{switch}$, so \cref{thm:sufficient_real} gives
collapse already when
$\Kstar(S_\mathrm{stop}+c_\mathrm{switch})\ge(\Kstar-1)(S_\mathrm{stop}+b\,c_\mathrm{switch})$,
i.e.\ $S_\mathrm{stop}\ge c_\mathrm{switch}\bigl((b-1)\Kstar-b\bigr)$ --- far weaker than
the crude $S_\mathrm{stop}>n\,b\,c_\mathrm{switch}$ of \cref{thm:setup_collapse}, and
instance-exactly superseded by \cref{thm:threshold}.
\end{remark}
...
\begin{proofbox}
\begin{proof}
%% TODO-VERIFY(Claude-Fable 2026-06-10): off-by-one fixed (K groups = K-1 stops under
%% the transition-sum convention of def. SSP-f; the old "K stops for K groups"
%% over-counted both sides by S_stop, leaving the conclusion unchanged).
A solution with $K$ groups (blocks) has $K-1$ configuration changes. The total
\emph{per-tool} switching cost of any solution is at most
$n\cdot b\cdot c_\mathrm{switch}$ (at most $b$ insertions per job, $n$ jobs). Hence the
\emph{best} $K$-group solution costs at most
$(K-1)S_\mathrm{stop}+n\,b\,c_\mathrm{switch}$, while \emph{any} $(K+1)$-group solution
costs at least $K\,S_\mathrm{stop}$. Under $S_\mathrm{stop}>n\,b\,c_\mathrm{switch}$:
\[
  K\,S_\mathrm{stop} \;>\; (K-1)\,S_\mathrm{stop}+n\,b\,c_\mathrm{switch}
  \;\ge\; \text{cost of the best } K\text{-group solution},
\]
so no $(K+1)$-group solution can beat the best $K$-group solution. Since this holds
for every $K\ge\Kstar$, the optimum is achieved at the minimum number of groups ---
exactly the JGP objective. \qed
\end{proof}
\end{proofbox}
...
is a (crude) upper bound on the \emph{collapse threshold}: below it the SSP and JGP
optima \emph{may} diverge --- whether they do is instance-specific, with exact
threshold $\rho_c(I)$ (\cref{thm:threshold}); e.g.\ $\rho_c=1\ll nb$ for the $6$-ring.
\end{remark}
...
%% TODO-VERIFY(Claude-Fable 2026-06-10): formula corrected. The earlier denominator
%% (K*-1)rho + Z_SSP paired K*-1 stops with the Z_SSP switch count, a combination NO
%% solution achieves (that is precisely the gap); it even contradicted the text's own
%% claim R(1)=1 (the old formula gives 6/5 at rho=1). The denominator must be the true
%% f_S optimum, min over K of (K-1)rho + z_K, with z_K the best switch cost among
%% K-block solutions (z_3=4, z_4=z_5=z_6=3 for the ring: verify_gap_doc.py).
Let $z_K(I)$ be the minimum U-SSP switch cost among solutions with exactly $K$
configuration blocks (so $z_{\Kstar}=H$ and $\min_K z_K=\ZSSP$). The JGP+GSP
approximation ratio for the $f_S$ objective at parameter $\rho$ is
\[
  R(\rho) = \sup_I\
  \frac{(\Kstar-1)\rho + H(I)}{\min_{K\ge\Kstar}\bigl[(K-1)\rho + z_K(I)\bigr]} .
\]
For the 6-job ring (\texttt{05\_jgp\_ssp\_gap\_analysis.tex}, Example~1):
$\Kstar=3$, $H=z_3=4$, $z_4=z_5=z_6=3$, hence
\[
  R(\rho)_{\text{ring}} = \frac{2\rho + 4}{\min(2\rho+4,\ 3\rho + 3)}
  = \begin{cases} \dfrac{2\rho+4}{3\rho+3}, & 0\le\rho\le1,\\[2pt]
     1, & \rho\ge 1.\end{cases}
\]
At $\rho=0$: ratio $=4/3$, decreasing to $1$ exactly at $\rho=\rho_c(\text{ring})=1$;
for $\rho \ge 1$ the JGP+GSP solution is exactly optimal for the $f_S$ objective.
...
%% TODO-VERIFY(Claude-Fable 2026-06-10): the numeric ranges above were attributed to
%% Privault1995; I could not verify they appear there --- treat as illustrative until
%% a source is found (cite removed pending check; ask the advisor for a practical
%% reference on magazine setup vs tool change times).
...
\]
%% TODO-VERIFY(Claude-Fable 2026-06-10): note --- this charges K stops for K groups
%% (initial setup included), unlike f_S above which charges K-1. The difference is a
%% uniform +S_stop shift in K, so argmin and the theorem below are unaffected, but
%% pick ONE convention before quoting numbers across sections.
...
\medskip
\textbf{(C1) not necessary.}
%% TODO-VERIFY(Claude-Fable 2026-06-10): argument corrected. Under the per-job
%% transition convention of def. SSP-f, constant f = epsilon makes EVERY solution
%% cost (n-1)epsilon (self-transitions are charged too), not (K-1)epsilon. The
%% conclusion survives: all solutions tie, so some optimum uses Z_JGP groups.
Let $f(C,C')=\varepsilon>0$ for all pairs including $C=C'$. Every solution then costs
$(n-1)\varepsilon$ (each of the $n-1$ job-to-job transitions costs $\varepsilon$,
whether or not the configuration changes), so all solutions are optimal --- in
particular one with $\ZJGP$ groups. Collapse holds while (C1) fails.
...
%% TODO-VERIFY(Claude-Fable 2026-06-10): caveat added --- this witness is DEGENERATE:
%% with 3 jobs and zero-slack singletons, every solution has K <= 3 = K*, so collapse
%% holds for ANY f on this instance. It still witnesses "collapse does not imply
%% (C2)", but a non-degenerate witness (an instance where K > K* solutions exist and
%% lose) would be more convincing; left as a small open task.
With only $3$ jobs no solution can use more than $3=\Kstar$ blocks, so collapse holds
trivially --- which is exactly the point: collapse holds while $f(C_1,C_2)=0$
violates (C2), so (C2) is not implied by collapse. (The witness is degenerate; a
non-degenerate witness with $K>\Kstar$ solutions available would strengthen this.) \qed
\end{proof}
\end{proofbox}
...
%% TODO-VERIFY(Claude-Fable 2026-06-10): strictness corrected (was "iff rho >
%% rho_c", contradicting the proof): at rho = rho_c the K*-group solution TIES the
%% best finer solution, so the optimum is still achieved with Z_JGP groups and
%% collapse (in the achieved-by sense of def:collapse) holds.
SSP-$f_S$ collapses to JGP if and only if $\rho\ge\rho_c(I)$.
\end{theorem}
\end{thmbox}
