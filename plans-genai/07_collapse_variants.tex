...
(v)~the necessity of the conditions — \textbf{open problem (TODO)}.
...
\begin{remark}[Necessity — open problem]
The sufficient conditions (C1)+(C2)+(C3) are \emph{not} proved to be necessary.
It is possible that $f$ causes collapse even when (C2) fails (e.g., if some
inter-group transitions have zero cost but the overall structure prevents benefiting
from more groups). Proving necessity, or finding a weaker necessary and sufficient
condition, is an open problem; see \cref{sec:todo}.
\end{remark}
...
\begin{propbox}
\textbf{Status: Proposed research direction.}
\end{propbox}

Define the ratio $\rho = S_\mathrm{stop} / c_\mathrm{switch}$.
As $\rho$ increases from 0 to $\infty$:
\begin{itemize}
\item $\rho = 0$: pure U-SSP; JGP and SSP optima can differ by up to
  $\ZJGP - 1$ switches (Theorem 2 of \texttt{05\_jgp\_ssp\_gap\_analysis.tex}).
...
\noindent\textbf{Proposed direction.}
Characterise the exact threshold $\rho_c$ at which the optimal number of groups
transitions from $\ZJGP$ to $\ZJGP+1, \ZJGP+2, \ldots$ This would give a complete
picture of how the JGP/SSP tradeoff depends on the physical parameters of the machine.
This is related to the Lagrangian relaxation of \texttt{06\_grouping\_selection.tex},
where $\lambda \approx \rho$ penalises extra groups.
