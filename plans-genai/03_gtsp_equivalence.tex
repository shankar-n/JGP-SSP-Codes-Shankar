...
\item \textbf{Depot cluster (path--cycle correction).}
%% TODO-VERIFY(Claude-Fable 2026-06-10): step ADDED — without it thm:bijection is
%% false: a GTSP Hamiltonian CYCLE pays the wrap-around arc, while the SSP is an
%% open path with a free initial load (e.g. two tool-disjoint jobs: path cost b,
%% any depot-free cycle cost 2b).
Add a singleton cluster $V_0^*=\{v_0\}$ with
$d^*(v_0,\cdot)=d^*(\cdot,v_0)=0$. The Hamiltonian cycle is forced through
$v_0$, which severs it into an open path with a free initial configuration ---
matching the free-initial-load SSP objective. Without this cluster the cycle
would additionally pay the return arc, and the equivalence below would fail.
\end{enumerate}
...
$v_{C,j}$ from each cluster $V_j^*$ and the depot $v_0$; cutting the cycle at
$v_0$ (both incident arcs cost $0$) and reading the replicas in order, collapsed
to their configurations, yields a configuration trajectory serving one job per
cluster of equal cost — a valid SSP sequence.
...
%% TODO-VERIFY(Claude-Fable 2026-06-10): direction corrected — the old text said
%% "the standard GTSP is a special case of the SSP", which is backwards and
%% contradicts the Metric Barrier section that follows.
The disjoint-cluster SSP is a \emph{native GTSP instance} (with the special metric
$d=b-|C_u\cap C_v|$): GTSP machinery applies directly, without the I-N overhead,
whenever instances are ``dense'' (large $|T_j|$ relative to $b$). The converse ---
which GTSP instances are expressible as SSPs --- is precisely the Metric Barrier
...
\begin{thmbox}
%% TODO-VERIFY(Claude-Fable 2026-06-10): statement weakened to what the proof
%% actually shows. The old claim ("can be polynomially reduced to a U-SSP only
%% if...") is FALSE as stated: both decision problems are NP-complete, so
%% polynomial reductions exist in both directions regardless of the metric.
%% What the proof establishes is a barrier for WEIGHT-PRESERVING embeddings.
\begin{theorem}[Metric Barrier --- weight-preserving embeddings]
\label{thm:barrier}
A GTSP instance with arc weight function $w$ admits a \emph{weight-preserving}
embedding into a U-SSP (i.e.\ vertices realised as configurations of a common
size $b$ with $w(u,v)=b-|C_u\cap C_v|$) only if $w$ satisfies all four
...
(iv)~the triangle inequality, proved in Part~II
(\texttt{02\_noncompact\_formulation.tex}, \S\,The Transition Metric).
...
\emph{Caveat (added 2026-06-10): the barrier concerns weight-preserving
embeddings only. Since both decision problems are NP-complete,
\emph{cost-distorting} polynomial reductions GTSP $\to$ SSP exist trivially;
whether a natural approximation-preserving reduction exists is open.}
\end{remark}
