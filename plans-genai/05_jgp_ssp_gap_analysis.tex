...
nor anywhere in the earlier exhaustive searches. The $\Kstar=3$ case is now
\emph{proved} for every $b\le4$ and, in general, outside an explicit corner: see
\cref{ssec:k3}, in particular \cref{thm:k3count}.
\end{openproblem}
...
\item[(b)] at $\Kstar=3$ the conjecture is open only in the corner
$2b\ge q+3R+6$ with $R\le q-2$ --- necessarily $b\ge5$ --- and there only outside
the mergeable case of \cref{thm:k3regions}(iii);
...
\begin{itemize}
\item \emph{Prove \cref{conj:gapK}}. The $\Kstar=3$ case is settled for every
$b\le4$ and, in general, outside the corner $2b\ge q+3R+6$, $R\le q-2$
(\cref{thm:k3count}); what remains is that corner ($b\ge5$, minus the mergeable
case of \cref{thm:k3regions}(iii)) and general $\Kstar\ge4$. The natural lever
for general $\Kstar$ is to extend \cref{lem:overlap} from triples to
$\Kstar$-tuples with the order-dependent wrap sum of \cref{lem:pathR}; the
counting identity $\sum x=2b-q-2c_3$ has a $\Kstar$-analogue, but the wrap sum
is no longer a plain minimum over orderings.
