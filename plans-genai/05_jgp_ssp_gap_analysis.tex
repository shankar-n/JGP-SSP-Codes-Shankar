...
nor anywhere in the earlier exhaustive searches. The first open case is $\Kstar=3$ for
general $b$: $\mathrm{gap}\le1$.
\end{openproblem}
...
\label{prop:43route}
Assume (a) \cref{conj:gapK}, and (b) every positive-gap $b=3$ instance has $\Kstar=3$ and
$\ZSSP\ge3$ (verified exhaustively on the $10691$-instance edge family, where positive gap
forces $\Kstar=3$, $|U|=6$, $\ZSSP=3$; open in general). Then \cref{conj:ratio} holds:
$H/\ZSSP\le4/3$ for $b=3$.
\end{proposition}
\end{thmbox}
\begin{proofbox}
\begin{proof}
...
\begin{itemize}
\item \emph{Prove \cref{conj:gapK}}, starting with $\Kstar=3$ (claim: $\mathrm{gap}\le1$
for \emph{all} $b$). By \cref{lem:costid} this says: some JGP-optimal grouping and order
forces at most $\Kstar-2$ more re-insertions than the SSP optimum.
