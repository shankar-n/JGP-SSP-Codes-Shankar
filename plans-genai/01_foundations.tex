...
\[
  ...
  \quad \forall k \in J.
\]
This model achieves the tightest LP relaxation among compact formulations \cite{Catanzaro2015}.

\subsection{Multi-Commodity Flow Model (SSPMF)}

A further tightening treats tool continuity as a network flow. For each tool
$t$, flow variable $f^t_{ij} \in \{0,1\}$ indicates that $t$ is continuously
retained across the direct arc $i \to j$. Flow conservation ties retention
to insertion cost:
\[
  z_{tj} \ge y_{tj} - \sum_{i \ne j} f^t_{ij}
  \quad \forall j, t.
\]
Arc capacity: $\sum_t f^t_{ij} \le b \cdot x_{ij}$.
This formulation explicitly tracks tool lifespans across the sequence, providing
tighter LP bounds than the basic arc-flow model, but still cannot fully escape
the symmetry problem described next \cite{Catanzaro2015}.
...
\begin{thmbox}
\begin{theorem}[LP bound collapse \cite{Tang1988,Catanzaro2015}]
...
\begin{remark}
The same collapse occurs in the arc-flow and multi-commodity models: the LP
solver can ``smear'' job assignments uniformly across all arcs, making every
tool appear at every node with fractional presence, eliminating all apparent
transitions.
\end{remark}
...
\begin{thmbox}
\begin{theorem}[$2^k$ 0-block symmetry \cite{Catanzaro2015}]
\label{thm:symmetry}
If a fixed sequence contains $k$ independent 0-blocks (whose simultaneous
retention does not violate capacity), then the compact model has at least
$2^k$ feasible integer variable assignments that all incur the same objective
cost, creating a flat plateau of size $2^k$ in the branch-and-bound tree.
\end{theorem}
\end{thmbox}

\begin{proofbox}
\begin{proof}[Sketch]
Each independent 0-block for tool $t_i$ spanning interval $[p_i, q_i]$
presents a binary choice $\delta_i \in \{0,1\}$: retain ($\delta_i=1$) or
evict-and-reload ($\delta_i=0$). By independence (capacity is not tight),
each choice $\delta_i$ is feasible regardless of the others.
Since the intervals are disjoint by independence, the objective cost is the
same for all $2^k$ combinations of $\{\delta_i\}$: evicting costs one
insertion (to re-load at $q_i+1$), but so does entering the 0-block having
not loaded the tool previously (one insertion at the first required position).
The branch-and-bound must distinguish these $2^k$ equivalent configurations
before proving optimality, causing exponential stalling. \qed
\end{proof}
\end{proofbox}
