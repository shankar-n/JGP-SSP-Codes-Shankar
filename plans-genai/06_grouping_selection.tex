...
\]
the lower bound $Z^{*}\!\ge|U|-b$ being met by a bidirectional-padding schedule. Thus
the one-directional MWHP value exceeds $Z_\mathrm{SSP}^{*}$ by exactly the slack $b-r$,
and the two coincide iff the groups are \emph{zero-slack} ($r=b$). (Values verified in
\texttt{plans-genai/\_verification/}: e.g.\ $K{=}3,r{=}3,\mathrm{ov}{=}1,b{=}4$ gives
$Z^{*}=3$, MWHP $=4$.)
\end{corollary}
