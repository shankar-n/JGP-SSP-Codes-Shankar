...
creating a ``double generation'' problem.
%% TODO-VERIFY(Claude-Fable 2026-06-10): pointer updated — the cluster-MTZ
%% "resolution" of Part IV is now PROVED NOT EXACT (job-domination counterexample,
%% Part IV); the working resolutions are per-configuration MTZ (exact, but rows
%% grow with generated columns) and the position-indexed formulations of Part X
%% (exact, FIXED O(n|T|) rows).
\emph{Update (2026-06-10): the cluster-MTZ remedy originally proposed in Part~IV is
proved \textbf{not exact} (Part~IV, job-domination counterexample). The current
resolutions are the per-configuration MTZ (exact; rows grow with generated columns)
and, preferably, the position-indexed formulations of Part~X
(\texttt{10\_position\_formulations.tex}): exact with a \emph{fixed} $O(n|T|)$
constraint set, eliminating row generation entirely.}
\end{mdframed}
...
\medskip
\textbf{The remedy.}
%% TODO-VERIFY(Claude-Fable 2026-06-10): rewritten — the cluster-MTZ remedy is
%% not exact (Part IV); Part X provides the exact fixed-row alternative.
The cluster-level MTZ replacement originally proposed for this purpose is
\emph{not exact} (Part~IV, settled negatively). Two correct remedies exist:
per-configuration MTZ (Part~IV, exact, but its row set grows with the generated
columns), and the position-indexed formulations of Part~X
(\texttt{10\_position\_formulations.tex}) — exact, with a \emph{fixed}
$O(n|T|)$ constraint set, so row generation disappears and column generation
proceeds against immutable rows. The LP-strength price and its repair (per-tool
counting rows; the PTF variant exceeding the SSPMF bound) are quantified there.
\end{mdframed}
...
\paragraph{Why generate configurations, not whole sequences.}
A tempting alternative is a Dantzig--Wolfe master over entire job sequences
$\sigma$: one column per sequence with cost $\mathrm{KTNS}(\sigma)$ and convexity
$\sum_\sigma \lambda_\sigma = 1$. This degenerates. Every sequence processes every
job exactly once, so each job-covering row $\sum_\sigma a_{j\sigma}\lambda_\sigma = 1$
is identical to the convexity row; the LP collapses to
$\min_\sigma \mathrm{KTNS}(\sigma) = Z^{*}_{\mathrm{SSP}}$ ---
%% TODO-VERIFY(Claude-Fable 2026-06-10): wording fixed: the bound is PERFECT, the
%% problem is that obtaining it requires solving the SSP itself in pricing.
a \emph{perfect} bound that is unusable, because attaining it requires the
pricing subproblem to solve the NP-hard SSP itself; the decomposition buys
nothing. Generating
\emph{configurations} (equivalently, arcs of the configuration graph), as below, instead
yields a genuine LP relaxation whose pricing is a tractable set-union knapsack.
\emph{(This observation was relocated here from the gap-analysis note, Part~V, where it
did not belong.)}
...
%% TODO-VERIFY(Claude-Fable 2026-06-10): honesty fix — ignoring the routing duals
%% makes the pricing a HEURISTIC, and the termination claim below must be weakened
%% accordingly (a configuration can have negative reduced cost through alpha/beta
%% even with zero coverage gain). Exact pricing must price ARCS, cf. Part X.
Ignoring the routing duals $(\alpha,\beta,\mu)$ reduces pricing to its dominant
coverage term --- a useful \emph{heuristic} oracle, but \textbf{not} an exactness
certificate (see the warning below). We seek:
\[
...
%% TODO-VERIFY(Claude-Fable 2026-06-10): certificate claim corrected — the SUKP
%% prices only the coverage term; LP optimality requires pricing the FULL reduced
%% cost including routing duals (alpha, beta, and the accumulated GSEC duals).
\textbf{Caution:} the SUKP of \cref{sec:sukp} prices only the coverage term; a
genuine optimality certificate must minimise the \emph{full} reduced cost
$\bar c(u,c,w)$, including the routing duals $\alpha_u,\beta_w$ (a join over
insertion pairs) and the accumulated GSEC duals. The clean way to obtain an exact
certificate with a fixed dual structure is the position-indexed master of Part~X,
whose pricing is a pure set-selection with no row churn. If a negative reduced
cost column is found, it is added to $V$ and the RMP is re-solved; integer
solutions are recovered by branch-and-price \cite{Barnhart1998}.
...
\item \textbf{The double-generation caveat.} The PCTSP-based formulation
  ...
  %% TODO-VERIFY(Claude-Fable 2026-06-10): updated (cluster-MTZ not exact).
  The cluster-MTZ shortcut of Part~IV is \emph{not exact}; the exact fixed-row
  resolution is Part~X (\texttt{10\_position\_formulations.tex}).
\end{enumerate}
