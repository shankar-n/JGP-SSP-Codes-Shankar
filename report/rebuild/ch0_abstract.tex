...
$1{,}421$ standard instances under a single one-hour limit, in twelve configurations
and $16{,}994$ runs. Where two methods both proved an optimum, they never disagreed:
$41{,}673$ such pairs, no exception. The results are consistent, and negative in an
informative way. The branch-and-Benders-cut solver closes $94\%$ of the instances on
which the counting bound is tight and $40\%$ of those on which it is not. Where it
fails, the incumbent is usually already optimal and only the proof is missing.
Fractional Benders cuts, which the earlier campaign could not test because of an
implementation defect, were generated in very large numbers once the defect was
corrected; sixty-seven million of them never raised the bound at termination on a
single instance, and cost sixty-seven solved instances. Of four strengthenings, only
the one that improves the incumbent helps. The branch-and-price prototypes measure the
relaxation directly: the position--configuration bound equals the counting bound on
every one of $2{,}114$ runs, and the position--transition bound exceeds it on three
runs out of $233$. The evidence is that a configuration-based exact method for the SSP
is limited by the strength of its bound and not by its implementation.
\end{abstract}
