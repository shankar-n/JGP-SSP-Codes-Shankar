...
needs. With $M_0=\varnothing$, the \emph{switch cost} of the schedule is the number
of tool insertions it makes,
\[
  \sum_{i=1}^{n}\lvert M_i\setminus M_{i-1}\rvert .
\]
The SSP asks for a feasible schedule of minimum switch cost. That minimum is
written $\Zst$.
\end{definition}
...
\end{array}
\]
Figure~\ref{fig:ring} draws it. Its optimum is $\Zst=6$ in the empty-start cost,
or $3$ once the first fill is not charged; Section~\ref{ssec:ktns} computes this.
\end{example}
...
interval matrices. Write $\KTNS(\pi)$ for the value the rule returns. Since the
rule is optimal for every fixed order,
\[
  \Zst \;=\; \min_{\pi}\ \KTNS(\pi),
\]
...
A \emph{configuration} is a set $C\subseteq T$ with exactly $\lvert C\rvert=b$
...
which the clusters \emph{overlap}: one configuration can serve many jobs. This
formulation is due to \citet{Colares2026Exact}. The overlap is what separates it
...
Section~\ref{ssec:compact} shows that the strongest compact formulation in the
literature has a linear relaxation exactly equal to it. It is therefore both an
elementary counting argument and the limit that twenty years of formulation work
has reached.
...
For every schedule that keeps the magazine as full as possible,
\[
  \text{cost}_{\mathrm{empty}}
  \;=\;\text{cost}_{\mathrm{free}}+\min(b,\lvert\U\rvert).
\]
\end{proposition}

\begin{proof}
An optimal loading keeps the magazine full, so the first position loads
$\min(b,\lvert\U\rvert)$ tools. The empty-start count charges these; the
free-initial count, by definition, does not; and every later insertion is counted
identically by both.
\end{proof}
