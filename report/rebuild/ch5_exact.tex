...
configurations, its position--transition variant carries a relaxation that provably
exceeds the coverage bound (Proposition~\ref{prop:ptf}), which is the one construction
in this report that does.
...
Three compact formulations from the literature serve as baselines. All three were
reimplemented for this study, and Section~\ref{ssec:validation} describes how they
were checked.

\citet{Catanzaro2015} give a family of five formulations in job-position and ordering
variables. Formulation~4 is used here. Formulations~3 and~4 have the same linear
relaxation and Formulation~4 is the smaller, which is the authors' own reason for
excluding Formulation~3 from their experiments. Formulation~5 is stronger still, and
the authors recommend it as the best of the family; it was not implemented in this
study, and Section~\ref{ssec:limits} records that as a limitation.

\citet{Laporte2004} formulate the problem with tool-state variables on a
travelling-salesman skeleton, with subtour elimination separated lazily and three
families of valid inequalities. \citet{daSilva2024} give a multicommodity-flow model
in which each tool routes one unit of flow through the position network. The last is
singled out by its relaxation.
...
In the first campaign this option was inert. Every cut it constructed was discarded by
an incorrect solver call, so no fractional cuts were recorded, and the campaign could
not test the idea. The defect was corrected, and Section~\ref{ssec:results-bbc}
reports what the corrected version does.
...
On an instance whose optimum exceeds both bounds of Corollary~\ref{cor:lb}, the PTF
relaxation returns $2.10$ against a coverage bound of $2$; on the $6$-ring it is tight
at $\Zst=3$. PTF is therefore the first relaxation in this family to improve on the
elementary bound, and it does so with a fixed, polynomial row set.
...
development environment, the learning components were implemented and validated on a
self-contained cut family, knapsack cover cuts, rather than on SSP instances. It has
not been evaluated on the SSP, and no claim is made about its effect on this problem.
It is reported here as work carried out during the internship, not as a result.

Section~\ref{ssec:further} explains why evaluating it is unlikely to be worthwhile
until the bound question of Section~\ref{sec:disc} is resolved.
