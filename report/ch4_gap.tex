...
The retained witnesses do not support a threshold based on slack between the magazine
and the largest job. In Proposition~\ref{prop:seed}, jobs $C$ and $D$ have size four and
fill the $b=4$ magazine; the $b=3$ witness above also contains a capacity-filling job;
and the connected $b=6$ family of Theorem~\ref{thm:connected-gap} contains jobs that
fill all six slots. \citet{Burger2015} report empirically, in an industrial setting,
that the decomposition degrades as this slack grows. That observation is useful
external context, but it is not a criterion or mechanism established by the witnesses
in this report.
...
Two things follow from Section~\ref{ssec:ktnsgap}. The first is that the practical value
of such a rule is smaller than the walk model suggests, since the KTNS step already
recovers the optimum on most instances where the walk model loses it. The second is
that a useful rule must target the still-uncharacterised structure of positive-gap
instances. The retained witnesses do not justify using magazine slack as that
criterion: the $b=3$ witness, the $b=4$ seed, and the connected $b=6$ family all
contain capacity-filling jobs. The empirical trend reported by \citet{Burger2015}
therefore remains external context rather than a criterion established here.
