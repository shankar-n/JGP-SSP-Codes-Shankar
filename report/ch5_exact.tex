...
coverage row on $87.4\%$ of runs. This places the solver in the lineage of logic-based
...
\begin{aligned}
  ...
  \text{(Cov)}\quad & \sum_{k=1}^{n-1}\,\sum_{(C,C'):\,T_j\subseteq C} z_{C,C'}^{k}
                    ...
                    & j&\in J;\\
  \text{(T)}\quad & \sum_{k=1}^{n-1}\sum_{(C,C'):\,t\notin C,\ t\in C'} z_{C,C'}^{k}
                    \;+\!\!\sum_{(C,C'):\,t\in C} z_{C,C'}^{1}\ \ge\ 1,
                    & t&\in\U.
\end{aligned}
\]
...
tail of a step or the head of the last one. The counting rows (T) are the transition
form of PCF's rows of the same name: every used tool is either already present in the
configuration at the first position or is inserted by some transition. They carry the
lower bound, and the proof of Proposition~\ref{prop:ptf} in
Appendix~\ref{app:proofs} uses them and nothing else; the implementation generates
them under the same name. As in PCF the rows number $O(n\lvert T\rvert)$ and are
fixed, while the transition columns are generated. Call this PTF.
...
The witnesses are the two instances of Observation~\ref{obs:ptf-strict} on which the
