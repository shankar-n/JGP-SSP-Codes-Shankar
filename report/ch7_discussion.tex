...
one, and always need $b\ge4$. It rests on a census of instances with three to five jobs,
because the optimum there is computed by exhaustion. Nothing in it shows the same holds
at industrial sizes, and the agreement with \citet{Burger2015} comes from a different
method rather than a wider range of sizes. This is the least secure empirical claim in
the report and the first a reader should discount. The structural results of
...
jobs. A linear relaxation will realise less, possibly much less, and no measurement
inside a branch-and-price run exists yet. Both families are measured the same way, so
the comparison between them survives. What follows safely is that the arc-supported
family is exhausted, since that is a claim about a ceiling; that the window family will
