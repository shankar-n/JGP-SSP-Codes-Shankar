...
organises much of the exact literature, and it also explains why the heuristic
...
report the branch-and-cut on instances with eight and nine jobs, and the branch-and-bound
...
\citet{Catanzaro2015} produced what were then the tightest compact models, a family of five
...
combined benchmark sets of \citet{Yanasse2009} and \citet{Catanzaro2015}.

For the three compact models reimplemented in this report, that is where the relaxation
story stops: the strongest of those baselines coincides with a counting argument that
fits in one sentence. \citet{daSilva2024}
...
That baseline set is not the whole compact literature. \citet{Akhundov2024} later
derive symmetry-broken position and job-group arc-flow models and strengthen their
JGSMF formulation with valid inequalities. Those inequalities improve its linear
lower bound and the resulting model outperforms SSPMF in their experiments. JGSMF was
not reimplemented here, so no result below ranks the campaign solvers against it.
...
2004 & \citet{Laporte2004}   & the LSS model; B\&C and B\&B & the Tang--Denardo relaxation is identically $0$; B\&B to $25$ jobs beats B\&C at $9$ \\
...
\bottomrule
\end{tabular}
\caption{The exact-method lineage for the SSP. SSPMF reaches the coverage bound
$q=\lvert\U\rvert-b$ when every declared tool is used \citep{daSilva2024}; the later
JGSMF model adds symmetry and strengthening inequalities \citep{Akhundov2024}.
Only the former is among the compact baselines reimplemented here.}
\label{tab:lineage}
\end{table}
...
The configuration view that unifies grouping and sequencing is documented by
\citet{Moreira2026}, who attributes the formulation to a 2024 personal communication
by Rafael Colares, Mauricio de Souza and Annegret Wagler. It casts the SSP as a
generalised travelling-salesman problem whose clusters overlap, and solves it by a
non-compact integer program with exponentially many transition variables, together
with generalised subtour-elimination constraints separated as lazy constraints. The
overlap is what separates the problem from the textbook generalised
...
group. \citet{Otiai2026} builds an exact branch-and-price for the Job Grouping
...
The hybrid genetic search (HGS) of \citet{Mecler2021} is the established benchmark
comparator, coupling an order-based genetic algorithm with local search. Its initial
population consists of random job orders improved by local search; it does not seed
from grouping solutions. More recently, the parallel-tempering method of
\citet{Almeida2025} was compared with HGS on twelve large Mecler sets: it improved the
best value on four, tied it on three, improved the reported mean on nine, and was faster
on all twelve. It is therefore safer to regard HGS as the benchmark comparator than as
the current unqualified state of the art.
...
\citet{Furrer2017} subsequently treat the closest fixed-order predecessor: arbitrary
processing and setup times with the number of stops minimised first and switches
second. Section~\ref{ssec:collapse} instead analyses variable job order and
configuration blocks under a scalar setup weight, and states why its high-setup
exactness result does not transfer automatically to KTNS post-processing.
...
The exact frontier has moved again recently. \citet{Akhundov2024} exploit the reversal
symmetry of \citet{Ghiani2010} systematically, deriving symmetry-broken position-based
and arc-flow formulations from a job-group representation and strengthening the latter
with inequalities that improve its relaxation. \citet{Legrand2025}, with Catanzaro among the
...
reproducible published results located for this report. An accepted IFORS~2026
abstract by \citet{Locatelli2026} claims a newer combinatorial branch-and-bound that
outperforms the prior frontier and solves open instances, but no archival paper or
code was available by the report's 30~August~2026 literature cut-off; it is therefore
recorded but not used as a comparison baseline.

The latter bounds are combinatorial and are computed inside the search rather than as
a linear relaxation. Recent progress therefore spans both strengthened compact models
and methods that leave the compact linear-programming setting: search with sharper
combinatorial bounds, decomposition with an exactly settled subproblem, and
relaxations engineered to exceed the counting bound. The two methods of
Section~\ref{sec:exact} take the last two of those routes.
...
polynomially solvable oracle, so each cut comes from an exactly solved polynomial-time
subproblem at every
callback.
...
industrial assessment \citep{Burger2015}, and a classical worst-case guarantee:
\citet{CramaKlundert1999} prove the construction to be a $b$-approximation. What it does
not have is an \emph{instance-sensitive} theory --- a bound that tightens on the
instance at hand, a characterisation of the instances where the construction is exact,
or any separation of the two readings the construction admits.
Section~\ref{sec:gap} builds one, on top of theirs.

Second, on the exact side the strongest of the three compact baselines reimplemented
here equals the coverage bound $q$ \citep{daSilva2024}, and the configuration
relaxation does not improve on it on most instances \citep{Moreira2026}. This campaign
therefore tests either a method that
...
is the position--transition route of Section~\ref{ssec:pos}. The comparison does not
include the strengthened JGSMF formulation of \citet{Akhundov2024}.

Third, the configuration view that works well for the Job Grouping Problem
\citep{Otiai2026} has no tractable exact counterpart for the SSP, because the natural
