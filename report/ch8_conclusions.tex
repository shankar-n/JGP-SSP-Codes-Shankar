...
\begin{itemize}[topsep=3pt,itemsep=2pt]
  ...
  different witness family from the one the walk model suggests, and that family is
  disconnected (Observation~\ref{obs:unbounded-true}).
...
inequality~\eqref{eq:window} has roughly three and a half times the remaining headroom at its
ceiling (Table~\ref{tab:ceilings}). That family has now been measured inside the master
