...
another and agree. For the tested Benders regimes, they indicate that weak
certification, rather than incumbent quality, is the dominant obstacle: the tested
search enhancements do not close the loose instances, and nothing built here improves
the bound materially. Two further measurements exist to make that reading
trustworthy rather than to support it, and both do: no two methods that proved the same
optimum disagree on its value, and the complete fixed-denominator comparison ranks
...
\paragraph{The regime the ablation omits.}
The single cheapest experiment this report leaves undone is integer-cut Benders with the
hybrid-genetic incumbent and without fractional separation. Both components were measured
in isolation and both are positive relative to the regime they were measured against, but
the combination was never run. It requires no new implementation --- only a tenth entry in
the existing configuration matrix --- and it is the first thing a continuation should do,
because the certified count of $724$ reported here is the best this solver was
\emph{measured} at rather than the best it was \emph{built} to reach.
