...
\begin{itemize}[topsep=4pt,itemsep=2pt]
  ...
  witness $W$ of Observation~\ref{obs:refute}; the pair $I_0$, $I_1$ of
  Observation~\ref{obs:noclutter}; the $\Kst=4$ instance; the smallest sub-optimal
  instance of Proposition~\ref{prop:seed}; the disconnected copies of
  Observation~\ref{obs:copies}; the connected construction of
  Theorem~\ref{thm:connected-gap}; the star and non-matroid instances of
  ...
  Corollaries~\ref{cor:zerogap} and \ref{cor:smallZ},
  ...
  \item \textbf{The fixed-configuration setup-cost collapse} of
  Theorem~\ref{thm:collapse}, by enumerating all feasible groupings, full configurations
  and all orderings and minimising the augmented objective directly. This also checks
  that the threshold is a walk-frame statement; it does not test KTNS post-processing
  as though it preserved the number of group changes.
...
Theorem~\ref{thm:connected-gap} supplying a different family for the other
reading.
...
A third finding concerns the campaign harness and analysis rather than the structural
results. The historical runner's incomplete resume key left $58$ planned pairs
unrecorded; the repository uses the full key, and same-protocol completion restored the
fixed denominator before aggregation (Section~\ref{ssec:bench} and
Appendix~\ref{app:repro}). The source audit also found that
the raw Catanzaro directory contained ten duplicate files and one ad-hoc test outside
the published $160$-instance benchmark; the analysis now asserts and excludes exactly
those eleven identities. Two further properties of the raw result files can silently
corrupt analysis --- repeated file names and declared but unused tools --- and both are
handled by the code that reads the results.
