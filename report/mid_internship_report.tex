...
method and answers most of both. \emph{How wrong can the decomposition be?} We
...
bound; and we determine its worst-case law quantitatively: an exact cost formula
when the batch number is three, bounds that are tight at every known extremal
instance, and an explicit instance showing that the natural candidate law
$\mathrm{gap}\le\Kst-2$ is false. At $b=3$ we prove the ratio bound $H/\Zst\le
...
real tooling data says they do. \emph{Can the same grouping view solve the SSP
...
where no current method has traction. Every proved statement appears as a
...
\begin{itemize}[noitemsep,topsep=2pt]
  ...
  \item \emph{The extremal law} (Section~\ref{ssec:law}): an exact formula for
  the heuristic cost at $\Kst=3$; quantitative gap bounds for every $\Kst$,
  tight at every known extremal instance; a certified counterexample, found
  exactly on the boundary of the proved regimes, eliminating the natural
  candidate $\mathrm{gap}\le\Kst-2$; and the $4/3$ ratio bound at $b=3$ proved
  for every $\Kst\le3$, its $\Kst\ge4$ case being the one surviving
  conjecture.
  ...
  quantifying the coverage bound's tightness on the standard suites ($47.9\%$
...
Appendix~\ref{app:proofs}. A reader pressed for time can follow the spine of the
report through the heuristic's definition (Section~\ref{ssec:jgp}), the exactness
theorem and the extremal law (statements in Sections~\ref{ssec:exact}
...
extremal law itself is determined at $\Kst=3$, including the certificate that
eliminates the natural candidate $\Kst-2$ (Section~\ref{ssec:law}). The
...
\subsection{The extremal law at $\Kst=3$}\label{ssec:law}
...
attains with equality. Third, a certificate: the natural candidate law
...
on the decomposition valid for every $\Kst$---on the $b=5$, $\Kst=4$ extremal
witness it gives $4$ where the transition cap gives $8$.
...
\begin{proposition}[The $\Kst-2$ bound fails in general]\label{prop:refute}
There are instances with $\Kst=3$ and gap $=2$. One, with $b=5$ and
$\U=\{0,\dots,8\}$:
\[
  T=\bigl(\{0,1,2,4,7\},\ \{0,4,5\},\ \{1,5\},\ \{2,6,8\},\ \{3,4,5,8\}\bigr),
\]
has $q=4$, $\Zst=4$ and $H=6$ (both values exhaustive, reproduced by two
independent implementations), hence $\mathrm{gap}=2>\Kst-2$ and $H/\Zst=3/2$.
The instance sits exactly on the boundary of the regimes of
Proposition~\ref{prop:k3}, $2b=q+3R_{\mathrm{opt}}+6$, and attains that
proposition's bound with equality:
$\min(q,\lfloor(2b-q)/3\rfloor)-R_{\mathrm{opt}}=2$.
\end{proposition}
\noindent Two consequences. First, the quantitative bounds of
Propositions~\ref{prop:k3} and~\ref{prop:genk} are not way-stations towards
$\Kst-2$: every known extremal instance---the $6$-ring at $b=3$, the $b=4$
witness, and the $b=5$ witnesses above---attains
$\min(q,\lfloor(2b-q)/3\rfloor)-R_{\mathrm{opt}}$ with equality, so whether
this expression is the exact worst case at $\Kst=3$ is now the natural open
question. Directed search sharpens its status: the bound is additionally
attained at $(b,q,R_{\mathrm{opt}})=(5,2,1)$ and $(5,3,1)$, while at the two
corners $(5,3,0)$ and $(6,3,0)$ extensive search finds only gap $1$ against
predicted caps of $2$ and $3$---if the expression is not the exact law, the
first deviation lives there. Second, the ratio picture recalibrates: $H/\Zst=3/2$ occurs already
at $\Kst=3$, so worst-case ratios are not governed by $\Kst$ alone, and no
uniform upper bound on the ratio (in $b$) is currently known.
...
answer---it cannot, for a structural reason the theory explains.
...
Table~\ref{tab:solves} reports instances solved to proven optimality over
instances attempted under the early-stop protocol (a configuration abandons a
set after eight consecutive failures on increasingly hard instances, so its
unattempted instances are at least as hard as those at which it stopped). All
optimality claims are mutually consistent: across the $806$ instances solved by
at least one method, no two methods disagree on the canonical cost.

\begin{table}[h]\centering
\begin{tabular}{lcc}
\toprule
Configuration & Primary ($411$) & Secondary ($1{,}010$)\\
\midrule
SSPMF                & $138/165$ & (not run, by design)\\
LSS                  & $91/98$   & $668/678$\\
CATZ-F4              & $91/99$   & $455/490$\\
BBC-LP (best Benders)& $1/9$     & $26/48$\\
BBC-K family         & $1/9$     & $0/8$\\
\midrule
PCF$'$ branch-and-price & \multicolumn{2}{c}{$94/379$ pooled, $n\le25$}\\
PTF branch-and-price    & \multicolumn{2}{c}{$72/351$ pooled, $n\le25$}\\
\bottomrule
\end{tabular}
\caption{Solved/attempted per configuration. Primary: $3600$\,s limit;
secondary: $600$\,s. The branch-and-price pools both sets on its validated
range $n\le25$; on the $n\ge30$ files (run separately) it solved none.}
\label{tab:solves}
\end{table}

\paragraph{Compact models.}
The multicommodity-flow model is the strongest method on the primary families,
replicating its state-of-the-art claim \citep{daSilva2024}: its relaxation
equals the coverage bound, so it closes bound-tight instances directly and
penetrates deepest into the hard tail. The striking secondary finding is the
$2004$ formulation of \citet{Laporte2004} under a modern solver: it matches
Catanzaro's F4 on the primary families (both halting at the same wall, the
$n=15$ instances with $m\ge30$, where memory or time is exhausted) and
dominates the secondary suite. Modern solver technology has largely erased the
$2004$--$2015$ formulation gap at classic sizes; published comparisons against
older formulations should be read with the solver generation in mind.

\paragraph{The Benders algorithm.}
The branch-and-Benders-cut solver finds the optimal schedule early but cannot
prove it: on capacity-loose instances (where every pairwise bound $w_{ij}$
vanishes) the master's dual bound remains frozen at its trivial root value
after the full hour, ${\sim}250{,}000$ nodes and ${\sim}75{,}000$ optimality
cuts, while on instances with informative root bounds it converges in minutes.
The ablation is one-sided: the dual-cut family is the only active axis
(LP-cut configurations solve $26$ of $48$ attempted secondary instances, the
combinatorial-cut family none); triplet bounds are neutral; and fractional
Benders cuts are \emph{structurally inactive}---zero violated cuts were found
across the campaign and in controlled runs with presolve disabled, consistent
with the loading subproblem's value collapsing to the pairwise bound at
fractional orderings. The method is bound-limited exactly in the sense of
Section~\ref{ssec:assess}.

\paragraph{The branch-and-price.}
PCF$'$ proves optimality on $94$ of $379$ attempted instances (about $15\%$ of
them at the root; median $89$ nodes), PTF on $72$ of $351$ with its heavier
pricer; neither reaches $n\ge30$. Consistent with
Proposition~\ref{prop:pcfprime}, the solves concentrate where the coverage
bound is tight, and the prototype's value is diagnostic rather than
competitive: it measures exactly where the configuration relaxation carries
information.

\paragraph{Bound tightness.}
Among the $806$ instances solved by any method, the coverage bound
$\lvert\U\rvert-b$ equals the optimum on $386$ ($47.9\%$)---an upper-biased
estimate, since unsolved instances skew loose. The dichotomy of
Section~\ref{sec:gap} is thus not a corner phenomenon: the standard benchmark
suites split roughly in half between the regime where the heuristic tends to
be exact and exact methods close at the root, and the regime where both must
work---which is where the position--transition relaxation and the planned
polyhedral work aim.
...
$\Kst-2$ is proved on wide regimes and \emph{refuted} outside them by an explicit
counterexample attaining our quantitative bounds with equality
(Propositions~\ref{prop:k3} and~\ref{prop:refute}), with the heuristic cost at
$\Kst=3$ given in closed form (Proposition~\ref{prop:hk3}). The circuit clutter
...
family for the JGP relaxation (Section~\ref{ssec:clutter}). On the algorithmic
...
theory and the extremal law (Sections~\ref{ssec:ratio}--\ref{ssec:law}), the
...
analysis (Section~\ref{sec:comp}). Three items remain. They are independent of
...
\paragraph{The exact extremal law.}
Every known extremal instance attains the bound of Proposition~\ref{prop:k3}
with equality, so the natural question is whether that expression is the true
worst case at $\Kst=3$. Alongside it stand the $\Kst\ge4$ regime
...
\paragraph{The polyhedral programme (with Prof.~Wagler).}
...
from there. Re-running the ablation with the corrected master is the fair
completion of the exact-methods comparison and is queued on the cluster.
