...
bound, a $b$-approximation guarantee, and two conjectures supported by exhaustive
enumeration. The second develops exact methods---a branch-and-Benders-cut algorithm
...
The SSP is inseparable from a simpler relative, the Job Grouping Problem (JGP), which
...
\citep{Colares2026Exact}. Two questions follow. First, how good is the heuristic---how
far can group-then-sequence be from the true optimum, and when is it exact? Second,
how well can the SSP be solved exactly through configurations---branch-and-price is
very effective for the JGP because its set-covering relaxation is tight
...
%==============================================================================
The group-then-sequence heuristic is the standard practical method for the SSP, yet how
far it can be from optimal has not been analysed. The obstacle is that the heuristic
...
The configuration formulation of Section~\ref{ssec:gtsp}, in the form proposed by
\citet{Colares2026Exact}, is the natural exact model over configurations, but as
originally written its subtour elimination is not valid; the correction, using
prize-collecting travelling-salesman constraints \citep{Moreira2026}, restores validity
at the price of \emph{exponentially many} constraints---one per subset of configurations.
To recover a polynomial constraint count, a Miller--Tucker--Zemlin reformulation
\citep{Miller1960,DesrochersLaporte1991} is the natural next step, but it does not escape
the dilemma.
...
counterexample would each settle the worst-case behaviour of the group-then-sequence
decomposition, for which no worst-case analysis is known (the approximation results
reported for the SSP concern other heuristics). Its constructive counterpart is the
...
%==============================================================================
What follows is not yet done; it sets out the remaining work and the order in which it
will be taken up over the balance of the project. Firm dates are left open---a substantial
mid-summer break intervenes---but the phasing is fixed by dependency, and the intention is
to conclude ahead of the project's formal end.

\paragraph{First, and already under way --- the computational campaigns.}
Two benchmark campaigns are running: the branch-and-Benders-cut ablation across its cut
families, and the position-indexed branch-and-price in its PCF and PTF variants, both
against the compact baselines and both recording the root relaxation alongside the
optimum. Completing them and their analysis is the immediate priority, since which method
merits further development turns on the resulting bound-tightness data.

\paragraph{Next --- the gap theory.}
With that data in hand, the central target is the gap bound $H-\Zst\le\Kst-2$
(Conjecture~\ref{conj:gap}): a proof, a proof of the $\Kst=3$ case for all $b$, or a
counterexample. In parallel, the enumerated pool of optimal groupings will be used to
attack the grouping-selection question (Section~\ref{ssec:grouping-sel})---whether a
grouping attaining the optimum can be found more cheaply than by solving the SSP.

\paragraph{Then --- stronger relaxations and the polyhedral view.}
Where the campaigns find the coverage bound loose, the work turns to the PTF relaxation,
characterising the instances on which it exceeds $\lvert\U\rvert-b$ and the valid
inequalities that strengthen it, and to the clutter, blocker, and hypergraph structure of
the configuration system behind the gap analysis (Remark~\ref{rem:perfect})---the newest
of the directions.

\paragraph{Throughout --- consolidation.}
The strongest of these threads will be consolidated toward a publication, with the
verification record kept current as the work proceeds.
