...
with zero-gap classes valid for every capacity, exact and quantitative gap laws at
$\Kst=3$ that are tight at every known extremal instance, a refutation of the
natural $\Kst-2$ gap conjecture, and a proof of the $4/3$ ratio bound at $b=3$
whenever $\Kst\le3$. The second develops exact methods---a branch-and-Benders-cut algorithm
...
are stated as conjectures; the large-scale benchmark is complete and reported.
\end{abstract}
...
\begin{itemize}[noitemsep,topsep=2pt]
  ...
  or $\lvert\U\rvert\le b+1$), an exact expression for the heuristic cost at
  $\Kst=3$, and quantitative gap bounds for every $\Kst$; we exhibit a family
  on which the gap grows linearly without bound; we \emph{refute} the natural
  conjecture $\mathrm{gap}\le\Kst-2$ by an explicit $\Kst=3$, $b=5$
  counterexample lying exactly on the boundary of the proved regimes and
  attaining our bounds with equality; and we prove the $4/3$ ratio bound at
  $b=3$ for every $\Kst\le3$, stating the remaining $\Kst\ge4$ case as the one
  ...
  the exact $\Kst=3$ expression does.
...
A second natural statement, suggested by every extremal instance known before
this work and by enumeration over $b\in\{2,3,4\}$, would bound the gap by
$\Kst-2$. It is true on wide regimes---for every $b\le4$ at $\Kst=3$ in
particular---but \emph{false in general}. The results below give the exact
heuristic cost at $\Kst=3$, quantitative bounds valid everywhere, the regimes
where $\mathrm{gap}\le\Kst-2$ does hold, and an explicit refutation just
outside them.
...
question. Second, the ratio picture recalibrates: $H/\Zst=3/2$ occurs already
...
sizes $x_{uv}$ are the data the clutter forgets. The clutter view retains its
force on the \emph{covering} side, where $\Kst$ itself lives
(Proposition~\ref{prop:chromK}); the polyhedral questions this raises are set
out in the planned work (Section~\ref{sec:planned}).
...
%==============================================================================
The methods of Section~\ref{sec:exact} have been implemented and benchmarked in
full; this section reports the design and the results.
...
vanishes identically (Theorem~\ref{thm:collapse}). Of the two natural conjectures,
...
(Section~\ref{ssec:clutter}). On the algorithmic
...
Phase~1 is complete; the aim is to finish the programme ahead of the project's
formal end.

\paragraph{Phase 1 (complete): the computational campaigns.}
Both campaigns---the full-factorial Benders ablation and the position-indexed
branch-and-price, against the compact baselines---ran to completion under the
early-stop protocol; the results and their reading are in
Section~\ref{ssec:results}. The bound-tightness split measured there
($47.9\%$ tight among solved instances) is the datum that steers the phases
below.

\paragraph{Phase 2: the exact extremal gap law.}
With the $\Kst-2$ bound refuted (Proposition~\ref{prop:refute}), the standing
question is the true extremal law. Every known extremal instance attains
$\min(q,\lfloor(2b-q)/3\rfloor)-R_{\mathrm{opt}}$
(Proposition~\ref{prop:k3}); deciding whether that expression is the exact
worst case at $\Kst=3$, tightening Proposition~\ref{prop:genk} at $\Kst\ge4$
(derandomising the ordering, sharpening the degree profile), and settling the
$4/3$ conjecture in its one remaining case $\Kst\ge4$
(Proposition~\ref{prop:43route} covers $\Kst\le3$) are the targets, with
enumeration directed at the boundary regimes where the counterexamples live.
In parallel, the exact expression of Proposition~\ref{prop:hk3} turns
grouping selection (Section~\ref{ssec:grouping-sel}) at $\Kst=3$ into a
concrete criterion, to be tested against the enumerated pool of optimal
groupings.
...
The polyhedral thread pursued with Prof.~Wagler now rests on the results of
Section~\ref{ssec:clutter}: the circuit clutter computes $\Kst$ but provably
cannot decide the gap (Proposition~\ref{prop:noclutter}), so the productive
questions are on the covering side---whether idealness of the covering system
characterises the instance classes on which the JGP relaxation is integral, and
whether the blocker's minimal obstructions localise the bridge tools that
generate re-insertions.
