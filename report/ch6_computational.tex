...
every configuration-based method built here except PTF, which exceeds it on three runs
of $233$, so a benchmark that records optima and bounds
...
every family is submitted for every configuration, and no instance is selected on the
basis of an outcome. Twelve configurations over $1{,}421$ instances is $17{,}052$
intended runs, and $16{,}994$ result rows were written, so $58$ produced no row at all
--- $39$ of \texttt{BBC-K+F}, $15$ of \texttt{CATZ-F4} and $4$ of SSPMF. These are
absent from the denominators of Table~\ref{tab:solves}, alongside the $427$ runs of
Section~\ref{ssec:validation} that ran and returned nothing.
...
tuning. The branch-and-price prototypes are run separately, under a thirty-minute limit
...
A run that survives at least seventeen minutes and is then killed without raising is
...
scheduler's accounting record for the job, which is not part of the result files. Nor
can the obvious size explanation be excluded. The crashed rows carry no size fields at
all, so $n\lvert T\rvert$ cannot be compared directly, and the family breakdown points
the wrong way for exoneration: $239$ of the $352$ crashes are Laporte5, the family with
$15$ jobs, against $101$ Laporte5 runs that completed, while the two families that never
crash are the two smallest. Size remains a live explanation for these losses.
...
there. Within one second the Benders configurations have closed between $54\%$ and
$59\%$ of everything they will close in the hour, against $46\%$ for SSPMF, $14\%$ for
\texttt{CATZ-F4} and $7\%$ for LSS: \texttt{BBC-K} has a median solve time of $0.06$
seconds against $59.8$ for \texttt{CATZ-F4}, because a master with a tight coverage row
...
minute and \texttt{BBC-LP} of \texttt{CATZ-F4} until $92$ seconds. No curve has stopped when the limit arrives: over
the last decade of the axis \texttt{BBC-LP} still gains $153$ instances and
...
instances in the first second and $726$ in the full hour, so more than half of
everything it will ever solve is solved in that first second. SSPMF starts lower
relative to its own total, at $482$ of $1{,}038$, and LSS lower still, at $57$ of
$844$. What the front-loading does \emph{not} mean is that the Benders curve has
stopped: between six minutes and the limit it still gains $153$ instances, a fifth of
...
\subsection{What the coverage bound predicts, and for which methods}\label{ssec:results-tight}
...
\midrule
bound tight ($\Zst=q$)  & $492/523$ ($94\%$) & $523/523$ ($100\%$)
                        & $372/515$ ($72\%$) & $372/378$ ($98\%$)\\
bound loose ($\Zst>q$)  & $234/592$ ($40\%$) & $515/592$ ($87\%$)
                        & $513/592$ ($87\%$) & $472/510$ ($93\%$)\\
\bottomrule
\end{tabular}
\caption{Success rate split by whether the coverage bound equals the optimum, on the
$1{,}115$ instances of known optimum. Only the Benders solver tracks the split: it loses
$54$ points across it. SSPMF loses $13$. \texttt{CATZ-F4} moves the other way, closing a
larger share of the loose instances than of the tight ones. LSS is shown for
completeness and should not be read as a fourth measurement --- its denominators are
reduced by the crashes of Section~\ref{ssec:validation}, and those losses fall on the
harder instances, so its row is biased upward. Denominators differ by method for the
same reason.}
\label{tab:tightness}
\end{table}
...
\begin{axis}[reportaxis, height=6.2cm,
  ...
  \addplot[black, mark=square, mark size=1.7, densely dotted]
    coordinates {(0,72.2)(1,78.6)(2,93.2)(3,87.1)(4,90.2)(5,75.0)(6,86.7)(7,96.7)(8,100.0)};
...
$43$ instances whose bound is loose by fifteen or more, both close $9.3\%$ --- four each.
\texttt{CATZ-F4} does neither. Its curve rises, and on those same $43$ instances it
closes all $43$, at a median of $93$ seconds. Whatever makes the right-hand end hard is
...
not track the bound at all: it closes $72\%$ of the tight instances and $87\%$ of the
loose ones, and every one of the $43$ where the bound is loosest.
...
Observation~\ref{obs:rootq}, nothing else on $87.4\%$ of runs; $-13$ for SSPMF, whose
...
the coverage bound is the dominant predictor of closure for a method whose dual
information reduces to that bound, and its predictive power falls away as the method
...
is that no implementation defect masks it on $2{,}114$ runs.
...
the bound is $2.0134$ against $q=2$, and Laporte3 \texttt{L11-6} and \texttt{L11-7},
...
optimum is the dominant predictor of whether the Benders solver closes an instance ---
a fact about that solver rather than about the instance, since \texttt{CATZ-F4} closes
all $43$ of the instances where the bound is loosest --- and where it fails the
