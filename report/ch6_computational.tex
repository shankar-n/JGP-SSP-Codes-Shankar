...
Numbers on their own settle nothing, so each subsection below states the question it
answers before the measurement that answers it.
Section~\ref{ssec:bench} fixes the protocol.
Section~\ref{ssec:validation} asks whether the implementations are sound, because every
comparison after it is worthless if they are not.
Section~\ref{ssec:results-solved} asks how far each method gets, which establishes the
ranking but explains nothing.
Section~\ref{ssec:results-bbc} asks whether the difficulty is finding good solutions or
proving them optimal, and whether the standard strengthenings move it.
Section~\ref{ssec:results-tight} asks what separates the instances a method closes from
those it does not, and finds a single structural property that does.
Section~\ref{ssec:results-bnp} asks the same question of the branch-and-price
prototypes, which can report their relaxation value directly and so answer it without
reference to solving times at all.
Section~\ref{sec:disc} then asks why the answer is what it is.
...
The Benders master requires the coverage row $\theta\ge\lvert\U\rvert$ of
Section~\ref{ssec:bbc} explicitly, because unlike every other method here it does not
carry that bound implicitly through a formulation. Omitting it costs the solver most of
its performance. This is reported because it quantifies the general point of
Section~\ref{sec:disc}: on this problem a missing bound is expensive in a way that a
missing search improvement is not.
...
different instance collection is itself a result. It closes $1{,}038$
...
rather than against another proof. That is the purpose of this subsection; the solve
counts are secondary and, for the reasons given next, weak evidence.
