...
\texttt{CATZ-F4} and $6\%$ for LSS: \texttt{BBC-K} has a median solve time of $0.06$
...
second and is never caught, while \texttt{BBC-K} stays ahead of LSS until $43$ seconds
and of \texttt{CATZ-F4} until $92$. No curve has stopped when the limit arrives: over
...
loose ones, and every one of the $43$ where the bound is loosest. So the sentence this
subsection can support is not that the coverage bound decides whether an instance is
hard. It is narrower and, for this report, more useful: \emph{the coverage bound decides
the outcome for a method whose bound is the coverage bound.} The Benders master is such
a method by construction --- Observation~\ref{obs:rootq} finds its root relaxation
sitting exactly on the coverage row on $87.4\%$ of runs --- and SSPMF is one on the
instances where every tool is used.
...
optimum decides whether the Benders solver closes an instance --- and decides it for
that solver rather than for the instance, since \texttt{CATZ-F4} closes all $43$ of the
instances where the bound is loosest --- and where it fails the incumbent is usually
already optimal. Of four strengthenings, only the one that
