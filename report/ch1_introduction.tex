...
well \citep{Burger2015}. \citet{CramaKlundert1999} analyse this construction and prove
...
The second is on the exact side. Every compact integer formulation surveyed in
Section~\ref{sec:sota} has a weak linear relaxation, and the strongest of them
\citep{daSilva2024} has a relaxation exactly equal to the number of tools minus the
magazine capacity, under their assumption that every tool is used. That is an
elementary counting bound: every tool that does not fit in the initial magazine must be
inserted at some point. So the strongest relaxation among the compact formulations
considered here is this counting argument, and a solver relying on it cannot prove
optimality where the bound is loose.
