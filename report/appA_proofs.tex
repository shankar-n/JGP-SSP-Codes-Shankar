...
recall that every configuration holds exactly $b$ tools. If $\lvert\U\rvert\le b$ then
$q=0$ and the second claim is immediate, so assume $\lvert\U\rvert>b$.
...
\emph{It can be strictly greater}, and this half is established by exhibiting witnesses
rather than by a general argument. Three appear in the campaign of
Section~\ref{ssec:results-bnp}, where the root relaxation is recorded on every run: on
Catanzaro \texttt{A0-0} it is $2.0134$ against $q=2$, and on Laporte3 \texttt{L11-6} and
\texttt{L11-7} it is $6$ against $q=5$. A witness is all the statement requires; how
large the excess can be made is Problem~\ref{prob:ptf-gap} and is open.
\end{proof}
